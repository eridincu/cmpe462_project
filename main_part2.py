...
def apply_models(key, y_val, processed_training_count, processed_validation_count, processed_training_tfidf, processed_validation_tfidf):
    ...
    start = time.time()
    MNB = MultinomialNB()
    MNB.fit(processed_training_count, y_train)
    # print ("Accuracy for M. Naive Bayes ngram: %s"
    #         % (accuracy_score(y_val, MNB.predict(processed_validation_count))))
    accuracy_dict_MNB_NGRAM[key] = accuracy_score(
        y_val, MNB.predict(processed_validation_count))

    MNB.fit(processed_training_tfidf, y_train)
    ...
    accuracy_dict_MNB_TFIDF[key] = accuracy_score(
        y_val, MNB.predict(processed_validation_tfidf))

    GNB = GaussianNB()
    GNB.fit(processed_training_count, y_train)
    # print ("Accuracy for G. Naive Bayes ngram: %s"
    #     % (accuracy_score(y_val, GNB.predict(processed_validation_count))))
    accuracy_dict_GNB_NGRAM[key] = accuracy_score(
        y_val, GNB.predict(processed_validation_count))

    GNB.fit(processed_training_tfidf, y_train)
    # print ("Accuracy for G. Naive Bayes tf/idf: %s"
    #     % (accuracy_score(y_val, GNB.predict(processed_validation_tfidf))))
    accuracy_dict_GNB_TFIDF[key] = accuracy_score(
        y_val, GNB.predict(processed_validation_tfidf))

    LR = LogisticRegression(max_iter=100000)
    LR.fit(processed_training_count, y_train)
    score = LR.score(processed_validation_count, y_val)
    accuracy_dict_LR_NGRAM[key] = score

    LR = LogisticRegression(max_iter=100000)
    LR.fit(processed_training_tfidf, y_train)
