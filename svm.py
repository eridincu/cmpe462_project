...
def apply_svm(y_train, y_val, key, training_data, validation_data, vocab_tfidf):
...
if __name__ == "__main__":
    ...
    # training data
    X_train, y_train =  initialize_data('TRAIN', 'resultsSVMLR/x_train.txt', 'resultsSVMLR/y_train.txt', X_train, y_train)
     # validation data
    X_val, y_val = initialize_data('VAL', 'resultsSVMLR/x_val.txt', 'resultsSVMLR/y_val.txt', X_val, y_val)
    ...
    max_features_list = range(2359, 2360,1)
    min_df_list = [9]
    max_df_list = [0.8]
    ...
    accuracy_dict_list = [accuracy_dict_SVM, accuracy_dict_LR]
    accuracy_dict_list_names = ['result_accuracy_dict_SVM', 'result_accuracy_dict_LR']
